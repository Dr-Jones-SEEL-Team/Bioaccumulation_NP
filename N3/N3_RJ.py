...
def RJ(x,y,yold,p,h):
    ...
    for i in index:
        if i==0 : #solid-biofilm interface node for unbound NP balance
            ...
            J[i,0]=1-h*(-2*gam/dx**2+2*(y[5]-y[2])/dx**2-F*(eps-y[1]));
            J[i,1]=-h*F*(y[0]+K)
            J[i,2]=-h*(-2*y[0]/dx**2)
            J[i,3]=-h*(2*gam/dx**2)
            J[i,5]=-h*(2*y[0]/dx**2)
        elif i==1 : #solid-biofilm interface node for bound NP balance
            R[i]=y[1]-yold[1]-h*(F*(K*y[1]-y[0]*(eps-y[1])))
            J[i,0]=-h*F*(y[1]-eps)
            J[i,1]=1-h*F*(K+y[0])
            ...
            R[i]=y[i]-yold[i]-h*F*(K*y[i]-y[i-1]*(eps-y[i]))
            J[i,i-1]=-h*F*(y[i]-eps)
            J[i,i]=-h*F*(K+y[i-1])
            ...
            l=int(i/3)
            R[i]=y[i]-yold[i]-h*((y[i+3]-2*y[i]+y[i-3])*(x[l]+gam)/dx**2+(y[i+3]-y[i-3])*(1+(y[i+5]-y[i-1])/(2*dx))/(2*dx)+y[i]*((y[i+5]-2*y[i+2]+y[i-1])/dx**2-F*(eps-y[i+1]))+F*K*y[i+1])
            J[i,i-3]=-h*((x[l]+gam)/dx**2-(1+(y[i+5]-y[i-1]))/(2*dx))
            J[i,i-1]=-h*(y[i]/dx**2-(y[i+3]-y[i-3])/(4*dx**2))
            J[i,i]=1-h*(-2*(x[l]+gam)/dx**2+(y[i+5]-2*y[i+2]+y[i-1])/dx**2-F*(eps-y[i+1]))
            J[i,i+1]=-h*F*(y[i]+K)
            J[i,i+2]=-h*(-2*y[i]/dx**2)
            J[i,i+3]=-h*((x[l]+gam)/dx**2+(1+(y[i+5]-y[i-1]))/(2*dx))
            J[i,i+5]=-h*(y[i]/dx**2+(y[i+3]-y[i-3])/(4*dx**2))
        elif i%3==1 : #interior biofilm nodes for bound NP balance
            l=int((i-1)/3)
            R[i]=y[i]-yold[i]-h*F*(K*y[i+1]-y[i-1]*(eps-y[i]))
            J[i,i-1]=-h*F*(y[i]-eps)
            J[i,1]=1-h*F*(y[i-1]+K)
