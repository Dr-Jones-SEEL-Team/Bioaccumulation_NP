...
def RJ(x,y,yold,p,h):
    ...
    for i in index:
        if i==0 : #solid-biofilm interface node for unbound NP balance
            ...
            l=int(i/3)
            R[i]=y[i]-yold[i]-h*((y[i+3]-2*y[i]+y[i-3])*(x[l]**2+gam)/dx**2+(y[i+3]-y[i-3])*(2*x[l]+beta*(y[i+5]-y[i-1])/(2*dx))/(2*dx)+y[i]*(beta*(y[i+5]-2*y[i+2]+y[i-1])/dx**2-F*(eps-y[i+1]))+F*K*y[i+1])
            J[i,i-3]=-h*((x[l]**2+gam)/dx**2-(2*x[l]+beta*(y[i+5]-y[i-1])/(2*dx))/(2*dx))
            J[i,i-1]=-h*(beta*y[i]/dx**2-(y[i+3]-y[i-3])/(4*dx**2))
            J[i,i]=1-h*(-2*(x[l]**2+gam)/dx**2+beta*(y[i+5]-2*y[i+2]+y[i-1])/dx**2-F*(eps-y[i+1]))
            J[i,i+1]=-h*F*(y[i]+K)
        ...
        """This is the linear diffusivity version of the code"""
        # if i==0 : #solid-biofilm interface node for unbound NP balance
        #     R[i]=y[0]-yold[0]-h*(2*gam*(y[3]-y[0])/dx**2+y[0]*(2*(y[5]-y[2])/dx**2-F*(eps-y[1]))+F*K*y[1])
        #     J[i,0]=1-h*(-2*gam/dx**2+2*beta*(y[5]-y[2])/dx**2-F*(eps-y[1]));
        #     J[i,1]=-h*F*(y[0]+K)
