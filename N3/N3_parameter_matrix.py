...
def parameter_matrix_generator(h,tol,t1,t2,nx,gam,F,K,eps,omega,ups,Kp):
    parameter_combos_count=len(h)*len(tol)*len(t1)*len(t2)*len(nx)*len(gam)*len(F)*len(K)*len(eps)*len(omega)*len(ups)*len(Kp) #Number of combination of parameters needed to test
    parameter_matrix = np.zeros((parameter_combos_count,12)) #Initialize matrix with new combo of paramters in each row. 9 is because there are 9 paramteres, can change if model changes
                                        ...
                                        for omega_i in omega: #Loop for ratio of electrokinetic nonsense
                                            for ups_i in ups: #Loop for other electrokinetic nonsense
                                                for Kp_i in Kp: #Loop for equilibrium parition coeffecient of NP between water and biofilm
                                                    parameter_matrix[k,:]=[h_i,tol_i,t1_i,t2_i,nx_i,gam_i,F_i,K_i,eps_i,omega_i,ups_i,Kp_i]
                                                    k=k+1
