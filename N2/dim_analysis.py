...
def dim_analysis(c_set,parameter_combos_count,parameter_matrix,dim_param,new_count_number,vn_N2,vn_Main_Code,vn_parameter_matrix_generator,vn_parameter_checker,vn_csv_generator,vn_method_of_lines,vn_RJ,perc_acc_matrix,vn_linear_fitting,machine_number):
    ...
    Dmin=dim_param[0] #Diffusivity of NP in biofilm [m^2/s]
    zeta=dim_param[0] #Zeta potential [Volts, -25 mV] 
    H= dim_param[0] #Biofilm thickness [m, 100 microns]
    kf= dim_param[0] #forward rate constant for binding kinetics [(m^3/s)*(m^3/kg)^(n-1)/(sites)]
    ct= dim_param[0] #total concentration of binding sites [sites/m^3]
    Kp= dim_param[0] #parition coeffeicent for NP across supernatant-biofilm interface [dimensionless]
    co= dim_param[0] #Concentration of nanoparticle in the supernatant [kg/m^3, 100 ug/L]
    kr= dim_param[0] #Reverse bidning rate constant [1/s]
    phim= dim_param[0] #maximum potential in the biofilm [V]
    ...
    for pc_i in np.arange(0,parameter_combos_count,1): #Begin for loop to plot the different model paramters using MOL 
        ...
        cb=ncb*ct #Convert dimensionless bound concentration vector to dimensional bound concentration vector
