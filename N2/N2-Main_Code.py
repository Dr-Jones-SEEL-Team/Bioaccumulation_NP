...
vn_N2=2.1 #See meta-data at bottom for details
vn_Main_Code=2.1 #See meta-data fro details
...
kf= 10**-17 #forward rate constant for binding kinetics [(m^3/s)*(m^3/kg)^(n-1)/(sites)]
ct= 10**16 #total concentration of binding sites [sites/m^3]
Kp= 1000 #parition coeffeicent for NP across supernatant-biofilm interface [dimensionless]
co= 0.0001 #Concentration of nanoparticle in the supernatant [kg/m^3, 100 ug/L]
kr= 10**-22 #Reverse bidning rate constant [1/s]
phim= 0 #maximum potential in the biofilm [V] <---- Set this to zero to zero out beta
n= 0.85 #Hill coeffecient [Dimensionless]
dim_param=[Do, Dmin, zeta, H, kf, ct, Kp, co, kr, phim, n]
...
"""Commnented out linear_fit function because it has been superceded by dim_analysis sub-function
...
report=dim_analysis(c_set,parameter_combos_count,parameter_matrix,dim_param,new_count_number,vn_N2,vn_Main_Code,vn_parameter_matrix_generator,vn_parameter_checker,vn_csv_generator,vn_method_of_lines,vn_RJ,machine_number,internal_export_path)
...
Overall N2.2 Computer Model Meta-data
Version 2.1

Changed From version N2.1 to N2.2 (2/14/2021 12:00 am):
    -Significant changes to model to make it capable of processing dimensional results


Main_Code File Meta-data
Version 2.1

changes from 1.5 to 2.1 (2/14/2021 12:00 am):
    -commented out Report Generator and linear fits sub-functions, two were replaced by single dim_analysis sub-function
